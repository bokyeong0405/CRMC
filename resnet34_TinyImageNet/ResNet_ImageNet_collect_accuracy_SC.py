...
def load_data(save_dir):
    ...
    return X_test_s, y_test_encoded

# 정규화된 데이터 불러오기
save_dir = "TinyImageNet/Processed_data"
X_test_s, y_test_encoded = load_data(save_dir)
...
test_ds = (
    preprocess_data(X_test_s, y_test_encoded)
    .batch(BATCH_SIZE)
...
def test_split_accuracy_2d(original_model, splitting_points, test_ds, error_rates_x, error_rates_y):
    ...
    for i, er_x in enumerate(error_rates_x):
        print("error rate x:", er_x)
        print("======================")
        for j, er_y in enumerate(error_rates_y):
            print("error rate y:", er_y)
            test_accuracy = 0.0
            count = 0
            for data, label in test_ds:
                output = data
